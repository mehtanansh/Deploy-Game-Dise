...
class VsAIHard extends StatefulWidget {
  static const routeName = '/VsAIHardPage';
  PlayerInfo currentPlayer;
  @override
  State<StatefulWidget> createState() {
    return _VsAIHardState();
  }
}

class _VsAIHardState extends State<VsAIHard> {
  var lc = "o";
  bool playerX = true;
  bool win = false;
  int moveCount = 0;
        ...
        if (!promptResult(winner)) {
          ...
          for (var i = 0; i < 3; i++) {
            for (var j = 0; j < 3; j++) {
              if (_matrix[i][j] == " ") {
                _dupmat[i][j] = "o";
                score = _minimax(_dupmat, 1, false);
                _dupmat[i][j] = ' ';
                if (score > bestScore) {
                  bestScore = score;
                  moveI = i;
          ...
          await Future.delayed(Duration(seconds: 1)).then((_) {
            setState(() {
              _matrix[moveI][moveJ] = "o";
              _dupmat[moveI][moveJ] = "o";
              lc = "o";
              turn = "X's Turn \n";
            });
          });
